...
def get_historical_data(api_key, plant_id, fujiformatdate, last_object, tag_ids,year,time, month,downloads_folder,workbooks_folder,fileyear,cover_name,filename,address,formatdate,FIT,PV):
    ...
    start_date = f"{fujiformatdate}T000000"
    date_only = last_object.date()
    date_only = date_only.strftime('%Y%m%d')
    end_date = f"{date_only}T000000"
        ...
        if 'list' in json_data:
            ...
            for entry in data_list:
                ...
                for tag_data in entry["data"]:
                    ...
                    row[f"ID_{tag_id}"] = value

                df_list.append(row)

            df = pd.DataFrame(df_list)
            if not df.empty:
                input_path_workbook = os.path.join(workbooks_folder, "保守点検報告書_ベース.xlsx")
                output_path_excel = os.path.join(downloads_folder, "{}{}月.xlsx".format(plant_id, fileyear))
                df.to_excel(output_path_excel, index=False)
                print(f"Data exported to {output_path_excel}")
                ...
                source_column_name1 = 'ID_1.3'
                source_column_name2 = 'ID_1.6'
                destination_sheet_name = 'PreValue'
                destination_column_name1 = '売電電力量(kWh)'
                destination_column_name2 = '日射量(kWh/m2)'
                df_destination = pd.read_excel(output_path_excel, sheet_name=destination_sheet_name)
                df_destination[destination_column_name1] = df[source_column_name2]
                df_destination[destination_column_name2] = df[source_column_name1]

                # Save the modified dataframe back to the Excel file
                with pd.ExcelWriter(output_path_excel, engine='xlsxwriter') as writer:
                    df.to_excel(writer, sheet_name='Sheet1', index=False)
                    df_destination.to_excel(
                        writer, sheet_name=destination_sheet_name, index=False)
                wb3 = load_workbook(output_path_excel)
                ...
                output_path_workbook = os.path.join(
                    downloads_folder, "{}{}.xlsx".format(plant_id, fileyear))
                wb1.save(output_path_workbook)
                df = pd.read_excel(output_path_workbook, sheet_name='Data')
                numbers_of_rows = len(df[destination_column_name2])
                ...
                try:
                        ...
                        if sheet_name in workbook.sheetnames:
                            ...
                            if sheet_name == '表紙 ':
                                ...
                                date_object = datetime.strptime(
                                    '{}-{}-{}'.format(month, first_day, year), '%m-%d-%Y')
                                last_object = datetime.strptime(
                                    '{}-{}-{}'.format(month, Last_date, year), '%m-%d-%Y')
                                today = date.today()
                                datecell = FS['J15']
                                ...
                                start_date_str = '{}-{}-{}'.format(
                                    month, first_day, year)
                                start_date = datetime.strptime(
                                    start_date_str, "%m-%d-%Y")
                                end_date_str = '{}-{}-{}'.format(
                                    month, numbers_of_rows, year)
                                ...
                                while current_date <= end_date:
                                    ...
                                    current_cell = sheet.cell(
                                        row=current_cell.row + 1, column=current_cell.column)
                    ...
                    ws = wb['発電実績']
                    values = Reference(ws, min_col=25, min_row=17,
                                       max_col=25, max_row=cfinalvalue)
                    values2 = Reference(ws, min_col=26, min_row=17,
                                        max_col=26, max_row=cfinalvalue)
                    barchart = BarChart()
...
def laplace(username, password, filename, time, month, fileyear, downloads_folder, workbooks_folder, params, year, address, cover_name, FIT, PV, formatdate):
    ...
    api_url = "https://services.energymntr.com/megasolar/{}/services/api/download/monthly.php".format(
        username)
    try:
        ...
        if response.status_code == 200:
            ...
            source_column_name4 = 'PCS1 交流電力量(kWh)'
            column_names = df.columns
            # Load the data from the destination sheet into a dataframe
            destination_sheet_name = 'PreValue'
            # Change this to the desired destination column name
            destination_column_name1 = '売電電力量(kWh)'
            destination_column_name2 = '日射量(kWh/m2)'
            df_destination = pd.read_excel(
                output_path_excel, sheet_name=destination_sheet_name)
            ...
            if username == "UAP0170823":
                ...
                with pd.ExcelWriter(output_path_excel, engine='xlsxwriter') as writer:
                    ...
                    df_destination.to_excel(
                        writer, sheet_name=destination_sheet_name, index=False)
            if username == "FOO0197257":
                df_destination[destination_column_name1] = df[source_column_name4]
                ...
                for sheet_name in sheet_names:
                    ...
                    if sheet_name in workbook.sheetnames:
                        ...
                        if sheet_name == '表紙 ':
                            ...
                            current_date = start_date
                            current_cell = sheet[start_cell]
                            write_date_on_column(
                                current_date, end_date, current_cell, initial_row, final_row)
                            SS = workbook.active
                            SS = workbook[sheet_name]
                            num_rows_to_delete = final_row - initial_row
                            SS.delete_rows(initial_row, num_rows_to_delete)
...
def download():
    ...
    fyear = int(year)
    fmonth = int(month)
    _, Last_date = calendar.monthrange(fyear, fmonth)
    ...
    def get_workbook_folder():
        ...
        if isFile == False:
            ...
            messagebox.showerror("エラー", "エクセルファイルが見つかりません。{}パスのベースエクセルシートを確認してください。".format(
    ...
    users1 = [
        {"username": "XGM0182319", "password": "MUcjx4kMvCxeMo9", "filename": "広野ソーラーパーク", "address": "福島県双葉郡広野町折木字東下4-18、外7筆","cover_name": "合同会社SS福島広野", "FIT": '=40', "PV": '=0.29*9372', "opening_date": "12-12-2019"},
        {"username": "CBU0173382", "password": "aiqh9gMgrfLvUgz", "filename": "田方郡函南町太陽光発電所", "address": "静岡県田方郡函南町軽井沢字浜井場295-8　他10筆","cover_name": "リニューアブル・ジャパン株式会社","FIT": '=40', "PV": '=312*0.275+7392*0.245', "opening_date": "9-26-2018"},
        {"username": "FOO0197257", "password": "rdR9YWWNeumoiEL","filename":"千歳新川低圧_運転監視報告書_","address":"北海道千歳市新川597 番1","cover_name":" ","FIT":'=18',"PV":'=192*0.4',"opening_date":"1-23-2020"},
        #{"username": "UWT0204482", "password": "WhEERJbfJsykUP9","filename":"黄金町低圧_運転監視報告書_","address":"北海道伊達市南小金町196-235","cover_name":" ","FIT":'=18',"PV":'=218*0.41',"opening_date":"1-23-2020"},
        #{"username": "ZQR019A647", "password": "9tjs7td9PR9uLkM","filename":"越智ソーラーシェアリング低圧_運転監視報告書_","address":"千葉県千葉市緑区越智町545-1,2","cover_name":" ","FIT":'=36',"PV":'=822*0.275',"opening_date":"4-10-2020"},
        ...
        # {"username": "MUI020F591", "password": "zr9qjgpwdtW4tRe","filename":"大郷町小学校_電力量実績_", "address":"大郷町小学校","cover_name":" ","report_name":"運転監視報告書","FIT":'=36',"PV":'=230*0.34',"opening_date":"12-12-2019"},
        # {"username": "XVK020F592", "password": "kWiEAKcpWc9kfoJ","filename":"大郷町中学校_電力量実績_", "address":"大郷町中学校","cover_name":" ","report_name":"運転監視報告書","FIT":'=36',"PV":'=235*0.34',"opening_date":"12-12-2019"},
        # {"username": "SXR020F590", "password": "zzxf4ge3LmKKRuz","filename":"大郷町文化会館_電力量実績_","address":"大郷町文化会館","cover_name":" ","report_name":"運転監視報告書","FIT":'=36',"PV":'=128*0.34',"opening_date":"12-12-2019"},
        # {"username": "IQH020C788", "password": "zmeEuMcL4zuYjt4","filename":"大郷町役場_電力量実績_","address":"大郷町役場","cover_name":" ","report_name":"運転監視報告書","FIT":'=36',"PV":'=288*0.465',"opening_date":"12-12-2019"}
    ...
    user2 = [
        {"plant_id": "S001","api_key": "c8ecfb719e1da5e658d95c000cfdf3059fbeff5ba1497a249b444675aed45857", "filename": "roadside_station", "address": "福島県双葉郡広野町折木字東下4-18、外7筆","cover_name": "合同会社SS福島広野","FIT": '=40', "PV": '=0.29*9372', "opening_date": "12-12-2019"},
        {"plant_id": "S002","api_key": "c8ecfb719e1da5e658d95c000cfdf3059fbeff5ba1497a249b444675aed45857", "filename": "yamaga", "address": "福島県双葉郡広野町折木字東下4-18、外7筆","cover_name": "合同会社SS福島広野","FIT": '=40', "PV": '=0.29*9372', "opening_date": "12-12-2019"},
        {"plant_id": "S003","api_key": "c8ecfb719e1da5e658d95c000cfdf3059fbeff5ba1497a249b444675aed45857", "filename": "yamaga_B", "address": "福島県双葉郡広野町折木字東下4-18、外7筆","cover_name": "合同会社SS福島広野","FIT": '=40', "PV": '=0.29*9372', "opening_date": "12-12-2019"},
        {"plant_id": "S004","api_key": "c8ecfb719e1da5e658d95c000cfdf3059fbeff5ba1497a249b444675aed45857", "filename": "akada", "address": "福島県双葉郡広野町折木字東下4-18、外7筆","cover_name": "合同会社SS福島広野","FIT": '=40', "PV": '=0.29*9372', "opening_date": "12-12-2019"}        
    ]
    ...
    for user in users1:
        ...
        date_object = datetime.strptime(
            '{}-{}-{}'.format(omonth, odate, oyear), '%m-%d-%Y')
        formatdate = date_object.strftime('%Y年%m月%d日')
        laplace(username, password, filename, time, month, fileyear, downloads_folder,
                workbooks_folder, params, year, address, cover_name, FIT, PV, formatdate)
        counter1 += 1
        if counter1 == 3:
    ...
    for user in user2:
        ...
        date_object = datetime.strptime(
            '{}-{}-{}'.format(omonth, odate, oyear), '%m-%d-%Y')
        formatdate = date_object.strftime('%Y年%m月%d日')
        fujiformatdate = start_date.strftime('%Y%m%d')
        # start_date = f"{fujiformatdate}T000000"
        last_object = datetime.strptime(
            '{}{}{}'.format(year, month, Last_date), '%Y%m%d')
        # date_only = last_object.date()
        # date_only = date_only.strftime('%Y%m%d')
        # end_date = f"{date_only}T000000"
        tag_ids = ["1.3", "1.6"]
        get_historical_data(api_key, plant_id, fujiformatdate, last_object, tag_ids,year,time, month,downloads_folder,workbooks_folder,fileyear,cover_name,filename,address,formatdate,FIT,PV)
        counter2 += 1
        sleep(60)
        if counter2 == 4:
            messagebox.showinfo("ダウンロードステータス", "ダウンロード完了")
...
def open_date_picker():
    ...
    container = tb.Frame(top, padding=20, borderwidth=4,
                         relief="solid", width=300, height=250)
    container.place(relx=0.5, rely=0.5, anchor=CENTER)
    global mydate
    mydate = tb.DateEntry(container, bootstyle="danger",
                          width=20, dateformat=r"%Y-%m-%d")  # Increase box size
    mydate.pack(pady=50)
...
# Create a frame to enclose the login elements
login_container = tb.Frame(
    root, padding=20, borderwidth=4, relief="solid", width=300, height=250)
